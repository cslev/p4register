...
// #define ENABLE_DEBUG_NETCAT_EGRESS
...
{
    bit<8> one_character;
    bit<8> closing_character; //EOL/NL char is '0a'
}
                ...
                inout standard_metadata_t standard_metadata)
{
    ...
    state parse_tcp
    {
        ...
        {
            16w0: accept;
            default: parse_tcp_with_payload;
        }
    }

    state parse_tcp_with_payload
    {
        transition select(hdr.tcp.dstPort)
        {
            16w4444: parse_netcat;
            default: accept;
        }
    }
...
#if defined(ENABLE_DEBUG_NETCAT) || defined(ENABLE_DEBUG_NETCAT_EGRESS)
control netcat_debug(in headers hdr,
                     inout metadata meta,
                     in standard_metadata_t standard_metadata)
{
    //define meta table
    table netcat_debug_table
    {
        //define keys to match
        key =
        {
            hdr.netcat.one_character:        exact;
            hdr.netcat.closing_character:    exact;
            meta.read_from_register:         exact;
        }
                  ...
                  inout standard_metadata_t standard_metadata)
{
    ...
    #ifdef ENABLE_DEBUG_NETCAT
        netcat_debug() netcat_debug_start;
    #endif //ENABLE_DEBUG_NETCAT
            ...
            else
            {
                ...
                if(hdr.ipv4.isValid())
                {
                    ...
                    if(hdr.ipv4.protocol == IP_PROTO_TCP)// && hdr.tcp.isValid())
                    {
                        ...
                        if(meta.tcp_metadata.payload_length_in_bytes > 0 && meta.tcp_metadata.payload_length_in_bytes < 3)
                        {

                            r.read(meta.read_from_register,0);

                            r.write(0, hdr.netcat.one_character);
                            meta.modified=1;
                            hdr.netcat.one_character = meta.read_from_register;
                            #ifdef ENABLE_DEBUG_NETCAT
                                netcat_debug_start.apply(hdr, meta, standard_metadata);
                            #endif //ENABLE_DEBUG_NETCAT
                 ...
                 inout standard_metadata_t standard_metadata)
{
    ...
    #ifdef ENABLE_DEBUG_NETCAT_EGRESS
        netcat_debug() netcat_debug_start;
    #endif //ENABLE_DEBUG_NETCAT_EGRESS
                ...
                else
                {
                    if(hdr.ipv4.isValid())
                    {
                        ...
                        if(hdr.ipv4.protocol == IP_PROTO_TCP)
                        {
                            ...
                            if(meta.tcp_metadata.payload_length_in_bytes> 0)
                            {
                                #ifdef ENABLE_DEBUG_NETCAT_EGRESS
                                    netcat_debug_start.apply(hdr, meta, standard_metadata);
                                #endif //ENABLE_DEBUG_NETCAT_EGRESS
...
control MyComputeChecksum(inout headers hdr, inout metadata meta)
{
    apply
    {
       ...
       (
           (hdr.tcp.isValid() && hdr.ipv4.protocol == IP_PROTO_TCP && meta.modified == 0x1),
           {
               hdr.ipv4.srcAddr,
               hdr.ipv4.dstAddr,
               8w0,
